...
const ProductDetail = () => {
  ...
  useEffect(() => {
    if (product) {
      const minQty =
        product.minOrderQty || MIN_ORDER_QTY[product.category] || 100;
      setQty(minQty);
    }
  }, [product]);
  ...
  if (needTransport && !deliveryAddress.trim()) {
    ...
    // Scroll to delivery address input
  ...
  // Default image
  const defaultImage =
    "https://images.unsplash.com/photo-1590075865003-e48277faa558?w=800";
      ...
      <div className="product-detail-container">
        ...
        <div className="product-info-section">
            ...
            <div className="stars">
              {[...Array(5)].map((_, i) => (
                <FiStar
                  key={i}
                  fill={
                    i < Math.floor(product.rating || 4.5) ? "#FFC107" : "none"
                  }
                  stroke="#FFC107"
                />
              ))}
          ...
          <div className="product-price-box">
            ...
            <div
              className={`stock ${product.quantity > 100 ? "in-stock" : product.quantity > 0 ? "low-stock" : "out-of-stock"}`}
            >
          ...
          <div className="moq-notice">
            ...
            <span>
              Minimum Order:{" "}
              <strong>
                {getMinQty().toLocaleString()} {product.unit}s
              </strong>
            </span>
          </div>
          ...
          <div className="product-seller-card">
            ...
            <div className="seller-info">
              ...
              <div className="seller-details">
                ...
                <p>
                  <FiMapPin size={14} /> {product.location}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="quantity-selector">
            ...
            {qty < getMinQty() && (
              <p className="moq-warning">
                ⚠️ Minimum order quantity is {getMinQty().toLocaleString()}{" "}
                {product.unit}s
              </p>
            )}
                ...
                <div className="vehicle-selection">
                  ...
                  <div className="vehicle-grid">
                    {Object.entries(VEHICLES).map(([key, vehicle]) => {
                      ...
                      const isRecommended =
                        key === suggestVehicle(product.category, qty);
                      ...
                      return (
                        ...
                        >
                          ...
                          <h5>
                            {vehicle.name.replace(vehicle.icon, "").trim()}
                          </h5>
                          <p className="vehicle-capacity">{vehicle.capacity}</p>
                          <p className="vehicle-cost">
                            ₹{cost.toLocaleString()}
                          </p>
                        </div>
                      );
                ...
                <div className="transport-cost-box">
                  ...
                  <div className="cost-row">
                    <span>
                      ...
                      {VEHICLES[selectedVehicle]?.perKmRate}/km):
                    </span>
                    <strong>
                      ₹
                      {(
                        distance * (VEHICLES[selectedVehicle]?.perKmRate || 0)
                      ).toLocaleString()}
                    </strong>
                  </div>
          ...
          <div className="total-price-box">
            <div className="price-breakdown">
              <div className="breakdown-row">
                <span>
                  Product Cost ({qty.toLocaleString()} × ₹{product.price})
                </span>
              ...
              {needTransport && (
                <div className="breakdown-row">
                  <span>
                    Transportation ({VEHICLES[selectedVehicle]?.name})
                  </span>
                  <strong>₹{transportCost.toLocaleString()}</strong>
                </div>
              )}
        ...
        <div className="order-modal">
          ...
          <div className="modal-section">
            ...
            <div className="modal-row">
              <span>Quantity</span>
              <strong>
                {qty.toLocaleString()} {product.unit}s
              </strong>
            </div>
          ...
          <div className="modal-total">
            ...
            <div className="modal-row">
              <span>Transport Cost</span>
              <strong>
                {needTransport ? `₹${transportCost.toLocaleString()}` : "Free"}
              </strong>
            </div>
          ...
          <button
            ...
            disabled={
              orderPlacing || (needTransport && !deliveryAddress.trim())
            }
          >
            {orderPlacing
              ? "Placing Order..."
              : `✅ Confirm Order - ₹${totalCost.toLocaleString()}`}
          </button>
        </div>
      </Modal>
    </div>
